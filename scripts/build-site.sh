...
# PYTHONPATH ensures src/leginfo is found without pip install – strongholding build
PYTHONPATH=src python3 scripts/build-research-index.py
PYTHONPATH=src python3 -m leginfo build-db || echo "build-db skipped (no snapshot DB or optional)"
PYTHONPATH=src python3 -m leginfo import-bills --session "$SESSION" || echo "import-bills skipped (egress restricted or optional)"
SITE_BASE="$SITE_BASE" PYTHONPATH=src python3 scripts/export-bills.py || echo "export-bills skipped"
...
PYTHONPATH=src python3 scripts/build-ai-graph.py || echo "build-ai-graph skipped (no DB, optional)"
node scripts/verify-built-corpus.mjs || echo "verify-built-corpus skipped"
